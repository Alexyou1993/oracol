...
class PostsApi {
  ...
  Future<List<Post>> listenForPosts(List<String> following) async {
    ...
    for (List<String> following in parts) {
      final QuerySnapshot snapshot = await _firestore.collection('posts').where('uid', whereIn: following).get();

      final List<Post> result = snapshot.docs.map((QueryDocumentSnapshot doc) => Post.fromJson(doc.data())).toList();
